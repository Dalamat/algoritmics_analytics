"""Add amonut_received to invoices_view

Revision ID: 2aabe47d52f0
Revises: 55dd87ca08b2
Create Date: 2023-07-08 15:50:22.189007
...
revision = '2aabe47d52f0'
down_revision = '55dd87ca08b2'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        AS SELECT invoices.id,
            ...
            COALESCE(payments.amount_received, 0) AS "получено по платежу с учётом возвратов"
        FROM invoices
            LEFT JOIN students ON students.id = invoices.student_id
            LEFT JOIN groups ON students.group_id = groups.id
            LEFT JOIN payments ON payments.payment_id = CAST(NULLIF(invoices.payment_id,'') AS int8);
    """)
    pass
